class DropBoxController {
  ...
  initFirebase() {
    const firebaseConfig = {
      apiKey: "AIzaSyBNaORkR-JMnC4tfb-cXl77z4cnhY38Qvg",
      authDomain: "dropbox-clone-6c870.firebaseapp.com",
      databaseURL: "https://dropbox-clone-6c870-default-rtdb.firebaseio.com",
      projectId: "dropbox-clone-6c870",
      storageBucket: "dropbox-clone-6c870.appspot.com",
      messagingSenderId: "882061158326",
      appId: "1:882061158326:web:f561839f7462832de24317"
    }
    ...
    this.getSelection().forEach( li => {
      ...
      promises.push(new Promise((resolve, reject) => {
        if(file.mimetype === 'folder') {
          this.removeFolderTask(this.currentFolder.join('/'), file.originalFilename, key)
          .then(() => { resolve({fields:{key}})})
    ...
    return new Promise((resolve, reject) => {
      ...
      folderRef.on('value', snapshot => {
        ...
        if(snapshot.exists()) {
          ...
          this.getFirebaseRef('Inicio').child(key).remove()
      }
      })
    })
  }
  ...
  deleteEvent() {
    this.btnDelete.addEventListener('click', () => {
      this.removeTaks().then( responses => {
  ...
  showModal(open = true) {
    this.snackModalEl.style.display = open? 'block' : 'none'
  }
  ...
  uploadProgress(event, file) {
    ...
    this.progressBarEl.style.width = loaded+'%'

    this.fileNameEl.innerHTML = file.name

    let timeLeft = ((100 - loaded) * timeSpent) / loaded
  ...
  formatTime(time) {
    ...
    let minutes = parseInt((time / 60000) % 60)
  ...
  renderNav() {
    ...
    this.currentFolder.map((folder, index) => {
      path.push(folder)
      ...
      if(e.shiftKey) {
        ...
        if(firstLi) {
          ...
          lis.forEach((el, index) => {
            ...
            if(firstLi === el) indexes.push(index) 
            if(li === el) indexes.push(index) 
